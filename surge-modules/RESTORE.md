...
托管母版、86400 秒更新、模块化分离等现行能力直接视为当前 Surge-1.0 架构的一部分，不再单列为历史版本。
...
- 红果旧测试模块、兼容实验工程和无效 VIP 脚本已从正式仓库清理，不参与恢复。
...
当前正式母版不使用独立 Information Panel。状态摘要直接位于 `[General]` 顶部，只保留正式版本、Stable 状态、节点数、节点中转链路、资源更新周期和状态日期。该摘要是静态恢复信息，不应加入实时 HTTP 检查、generic script、MITM 或 Rewrite。
...
其中包含 Surge-1.0、7 个正式模块、恢复说明、上游依赖清单、Project Instructions 与当前 stable 脚本快照。旧备份、重复导航和历史副本不再作为正式恢复来源。
