 ...
 * v1.6: support binary-body-mode Uint8Array bodies confirmed by Surge diagnostics.
 */
...
function bodyToText(v){if(typeof v==='string')return v;if(v&&typeof v==='object'&&typeof v.length==='number'){try{if(typeof TextDecoder!=='undefined')return new TextDecoder('utf-8').decode(v)}catch(_){}try{return Array.from(v).map(x=>String.fromCharCode(x)).join('')}catch(_){}}return''}
...
const ids=found.map(x=>x.id), ct=(header('content-type').split(';')[0]||'unknown'), timestamp=new Date().toLocaleString(), binary=body&&typeof body==='object'&&typeof body.length==='number';
const bodyInfo=binary?`body=Uint8Array bytes=${body.length} decoded=${raw.length}`:`body=${typeof body} len=${raw.length}`;
...
const sig=(ids.length?'id|':'diag|')+message.replace(/^时间:.*\n/,'');let last={};try{last=JSON.parse($persistentStore.read(KEY)||'{}')}catch(_){}const now=Date.now();if(last.sig!==sig||now-Number(last.time||0)>15000){try{$persistentStore.write(JSON.stringify({sig,time:now}),KEY)}catch(_){}try{$notification.post('淘宝商品ID探测',ids.length?'命中商品ID':'已命中埋点请求',ids.length?ids.slice(0,5).join(' / '):`${bodyInfo}｜${ct}`)}catch(_){}}
$done({});
