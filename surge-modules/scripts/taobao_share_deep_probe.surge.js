/* Taobao share payload structure probe for Surge v1.16.1
 * Preserve the rich request-body structure; tiny responses must not overwrite the panel.
...
const PANEL='taobao_analytics_probe_panel', LAST='taobao_share_probe_last', SNAP='taobao_share_request_snapshot_v1161';
...
function summarizeForm(raw){const arr=[];for(const p of String(raw||'').split('&').slice(0,120)){const i=p.indexOf('=');if(i<=0)continue;let k=p.slice(0,i),v=p.slice(i+1);try{k=decodeURIComponent(k)}catch(_){};const d=dec(v);arr.push({k:safeKey(k),t:typeOfValue(d),len:d.length})}return arr}
function collectJsonKeys(v,out,prefix,depth){if(depth>5||v==null)return;if(Array.isArray(v)){out.push(`${prefix||'$'}[](${v.length})`);for(const x of v.slice(0,4))collectJsonKeys(x,out,prefix+'[]',depth+1);return}if(typeof v==='object'){for(const k of Object.keys(v).slice(0,100)){const x=v[k], p=(prefix?prefix+'.':'')+safeKey(k);if(x==null)out.push(p+':null');else if(typeof x==='object')out.push(p+':'+(Array.isArray(x)?'array':'object'));else out.push(p+':'+typeof x+'('+String(x).length+')');collectJsonKeys(x,out,p,depth+1)}}}
...
const keyLines=form.slice(0,22).map(x=>`${x.k}: ${x.t}(${x.len})`);const jsonLines=[...new Set(jsonKeys)].slice(0,24);const suspicious=form.filter(x=>/share|link|url|item|goods|auction|detail|target|page|scene|spm|args?|param|data|payload|event|ext|content|biz/i.test(x.k)).slice(0,14).map(x=>x.k);
const snap={time:Date.now(),host,path,rawLen:raw.length,decodedLen:decoded.length,formPairs:form.length,jsonOk,ids:ids.slice(0,5),suspicious,keyLines,jsonLines};
if(!resp && raw.length>20){try{$persistentStore.write(JSON.stringify(snap),SNAP)}catch(_){}}
let show=snap;if(resp && raw.length<=64){try{const saved=JSON.parse($persistentStore.read(SNAP)||'null');if(saved&&Date.now()-Number(saved.time||0)<120000)show=saved}catch(_){} }
const msg=`时间: ${new Date().toLocaleString()}\n${show.ids&&show.ids.length?'✅ 候选商品ID: '+show.ids.join(' / '):'已锁定分享请求体｜优先显示REQ结构'}\nREQ ${show.host}${show.path}\nbody=${show.rawLen} decoded=${show.decodedLen} formPairs=${show.formPairs} jsonRoot=${show.jsonOk?'yes':'no'}\n\n可疑字段名:\n${show.suspicious&&show.suspicious.length?show.suspicious.join(', '):'未发现明显 share/url/item 字段'}\n\nForm字段结构:\n${show.keyLines&&show.keyLines.length?show.keyLines.join('\n'):'无'}${show.jsonLines&&show.jsonLines.length?'\n\n嵌套JSON结构:\n'+show.jsonLines.join('\n'):''}`;
try{$persistentStore.write(JSON.stringify({message:msg,ids:!!(show.ids&&show.ids.length),time:Date.now()}),PANEL)}catch(_){}
